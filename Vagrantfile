...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.define "master" do |master|
    ...
    master.vm.provision :file, :source => "master.yaml", :destination => "/tmp/vagrantfile-user-data"
  ...
  config.vm.define "node01" do |node01|
    ...
    node01.vm.provision :file, :source => "node.yaml", :destination => "/tmp/vagrantfile-user-data"
  ...
  config.vm.define "node02" do |node02|
    ...
    node02.vm.provision :file, :source => "node.yaml", :destination => "/tmp/vagrantfile-user-data"
